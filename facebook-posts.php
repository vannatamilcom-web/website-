...
try {
    ...
    foreach (($apiResponse['data'] ?? []) as $item) {
        ...
        $posts[] = [
            'id' => (string) ($item['id'] ?? uniqid('fb_post_', true)),
            'message' => trim((string) ($item['message'] ?? '')),
            'fullPicture' => isset($item['full_picture']) ? (string) $item['full_picture'] : null,
            'createdTime' => isset($item['created_time']) ? (string) $item['created_time'] : null,
            'permalinkUrl' => isset($item['permalink_url']) ? (string) $item['permalink_url'] : null,
        ];
    }
    ...
    if ($payload === false) {
        throw new RuntimeException('Failed to encode Facebook posts response.');
    }
    ...
    echo json_encode([
        'generatedAt' => gmdate(DATE_ATOM),
        'posts' => [],
        'error' => 'Unable to load Facebook posts right now.',
    ], JSON_UNESCAPED_SLASHES);
}
